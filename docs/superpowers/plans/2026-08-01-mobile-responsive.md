...
По факту выполнения: `LeaderboardRow` переносился на вторую строку аккуратно, без наложений — contingency-фикс не потребовался, коммита по Task 4 нет.

---

## Задачи 5-8: найдено в ходе QA (Task 4), не было в исходном плане

Во время Task 4 обнаружился реальный баг: `html { font-size: 150%; }` в `src/index.css` делает все rem-значения в 1.5 раза крупнее номинала, из-за чего несколько `px-10`/`text-[Nrem]` без мобильного варианта на самом деле переполняли 375px. Баг чинился один и тот же (`px-10` → `px-6 sm:px-10`, аналогично для шрифта — база меньше, `sm:` восстанавливает прежнее значение), но обнаруживался по одному компоненту за раз при проверке — отсюда 4 доп. задачи вместо одной:

- **Task 5** — `src/components/home/HomeHero.tsx` (секция + заголовок `h1`), коммит `e537c8e`.
- **Task 6** — `src/components/home/ActiveTournaments.tsx`, коммит `8db0e1b`.
- **Task 7** — `src/components/home/SeasonBanner.tsx` (2 места: секция + подпись поверх баннера) и `src/components/home/FeaturesRow.tsx`, коммит `78773b3`.
- **Task 8** — `src/pages/SeasonsListPage.tsx` (страница «Сцены»), коммит `2b4091f`. Заголовок `h1` на этой странице измерен (`getBoundingClientRect`) и оставлен без изменений — переполнения не было.

После Task 8 повторный `grep -rn "px-10" src/` не нашёл ни одного необёрнутого в `sm:` вхождения нигде в кодовой базе.

**Осознанно не в рамках этой ветки:** финальное ревью нашло, что правая часть шапки (`Header.tsx`, тема/колокольчик/кнопка входа) переполняется для неавторизованных пользователей ниже ~360px — но не при 375px, которые пользователь явно выбрал как целевую минимальную ширину при брейнсторминге (а не 320px). Это отдельная, вне рамок этой ветки находка.

---

## Итоговая проверка плана

- [x] `npm run build` проходит на финальном состоянии ветки.
- [x] Все 4 задачи закоммичены отдельными коммитами (кроме Task 4 — contingency-фикс не понадобился).
- [x] Задачи 5-8 (найдены в ходе Task 4) реализованы, проверены (implement → spec review → code quality review) и закоммичены.
- [x] Финальное ревью всей ветки пройдено, вердикт — готово к мержу.
- [x] Ветка `feat/mobile-responsive` готова к код-ревью/PR.
